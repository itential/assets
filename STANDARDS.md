...
- **One auth method defined**: Only one `securityScheme` should be defined. Itential Platform supports a single authentication method per integration instance, so additional schemes will not be usable. If a vendor's API supports multiple incompatible auth methods (e.g., an API key header vs. a Bearer token), publish separate specs rather than combining schemes in one spec.
    - **Name the auth method into every variant's filename, not just the non-default one**: once a spec exists in more than one auth variant, none of them is the implicit default — give every variant's filename an auth-method qualifier in the snake_case title portion, e.g. `cisco_meraki_dashboard_api_key-latest.json` vs. `cisco_meraki_dashboard_bearer-latest.json`, not `cisco_meraki_dashboard-latest.json` vs. `cisco_meraki_dashboard_bearer_variant-latest.json`. This also applies retroactively — if a second auth variant is added later, rename the original (now-unlabeled) file to include its own qualifier in the same contribution, and update the product's `README.md` links accordingly (per the "No duplicate specs" rule above).
    - **Lead each variant's `info.description` with its auth method** (e.g. "Authentication: OAuth 2.0 (3LO)..." or "Authentication: HTTP Basic..." as the first sentence): `info.description` is what's shown in the Itential Platform GUI's model list, and per the shared-model-identity pattern below, `info.title` alone won't distinguish variants there — the description becomes the only visible differentiator.
    - **Shared model identity across auth variants**: if a Studio Project should work interchangeably no matter which auth variant is imported (e.g., a customer can choose Basic Auth or OAuth 2.0 depending on their environment, and the same workflows should run either way without editing task references), give the variant specs identical `info.title` and `info.version`, differing only by filename (per the auth-qualifier pattern above). Itential Platform identifies an Integration Model by title+version, so matching these lets a Studio Project's task `locationType`/`app` resolve correctly against whichever variant is actually imported on a given platform. Document this explicitly in the product's `README.md`, including that only one of the identically-identified variants can be imported into a given platform at a time — attempting both is Itential Platform's normal "model already exists" collision, a deliberate consequence of this pattern, not a bug.
