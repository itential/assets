...
This is a working example: a Python plugin for Delinea Secret Server Cloud, the registration steps, and how to reference the resulting alias — from device inventory or from an Integration Model instance.
...
Integration Model instance         ├─▶  resolves $GATEWAYSECRET_(alias)  ──▶  delinea-plugin.py  ──(OAuth2 password grant)──▶  Delinea Secret Server Cloud
(Gateway-executed)                ─┘
```
