...
| [Studio Projects/](./Studio%20Projects/) | Itential Platform - Studio project containing all 48 workflows in 7 folders |
...
|---|---|---|---|
| [`itential_platform_studio-latest.json`](./OpenAPIs/itential_platform_studio-latest.json) | latest | 48 | Studio — see breakdown below |
...
Backed by the **`Itential Platform - Studio:latest`** Integration Model (see [`itential_platform_studio-latest.json`](./OpenAPIs/itential_platform_studio-latest.json) above). The project contains **48 workflows** organized into **7 folders**.

#### Folder Structure

| Folder | Workflows | Scope |
|---|---|---|
| Automation Studio - Workflows | 8 | Create Workflow, Update Workflow, List Workflows, Get Workflow Detail by Name, Get Multiple Task Details, Get Task Details, ... |
| Workflow Builder | 8 | Save Workflow, Export Workflow, Import Workflow, Rename Workflow, Get Task Details, Get Tasks List, ... |
| Workflow Engine | 2 | Validate Stored Workflow, Validate Workflow Object |
| Operations Manager - Jobs | 11 | Start Job, Get Job, List Jobs, Cancel Jobs, Pause Jobs, Resume Jobs, ... |
| Operations Manager - Tasks | 8 | Get Task, List Tasks, Assign Task, Claim Task, Release Task, Retry Task, ... |
| Method of Procedures - Templates | 7 | Create Command Template, List Command Templates, Update Command Template, Delete Command Template, Import Command Template, Export Command Template, ... |
| Method of Procedures - Run Commands | 4 | Run Command Against a Device, Run Command Against Multiple Devices, Run Command Template Against Devices, Run a Single Command From a Template |

A handful of workflow names (`Bulk Delete Jobs`, `Delete Job`, `Get Job`, `List Jobs`, `List Tasks`, `List Workflows`, `Update Workflow`) are prefixed with `Itential Platform - Studio` to avoid colliding with identically-named workflows already published for other products — workflow names are unique across the whole Itential Platform instance, not scoped per-project.
